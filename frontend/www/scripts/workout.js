...
async function retrieveWorkout(id) {
  ...
  if (!response.ok) {
    ...
    /// WORKOUT FILES
    async function fileClickLink(event, fileInfo) {
      event.preventDefault(); // Not redirect immediately
      await sendRequest("GET", fileInfo.file)
        .then((result) => result.blob())
        .then((x) => showFile(x, fileInfo));
    }

    function showFile(blob, fileInfo) {
      ...
      urllink.href = data;
      let pathArray = fileInfo.file.split("/");
    ...
    for (let file of workoutData.files) {
      ...
      //a.href = file.file;
      a.href = "";
      a.onclick = (event) => fileClickLink(event, file);
