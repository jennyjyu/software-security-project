...
// function called when a fileLink is clicked
async function fileClickLink(event, fileLink) {
  event.preventDefault(); // Not redirect immediately
  console.log("file-info", fileLink);
  await sendRequest("GET", fileLink)
...
async function displayFiles() {
  ...
  for (let fileUrl of user.coach_files) {
    ...
    aFile.href = "";
    aFile.onclick = (event) => fileClickLink(event, file.file);
